...
const ProductCard = ({ product }: { product: any }) => {
    ...
    if (product?.reviews?.length > 0) {
        productRating = product.reviews.reduce((acc: number, item: any) => acc + item.rating, 0) / product.reviews.length;
    }

    return (
        <div
            onClick={() => router.push(`/product/${product.id}`)} // BAŞINDA SLASH VAR!
